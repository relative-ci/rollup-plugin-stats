...
describe('package - vite options', () => {
  ...
  test('should output stats JSON file', async () => {
    ...
    const actual = await fs.readFile(path.join(__dirname, config.output.dir, 'stats.json'), 'utf8');
  ...
  test('should output stats JSON file with custom relative filename', async () => {
    ...
    const actual = await fs.readFile(path.join(__dirname, relativeFileNameConfig.output.dir, '../artifacts/stats-relative-path.json'), 'utf8');
    const stats = JSON.parse(actual);
